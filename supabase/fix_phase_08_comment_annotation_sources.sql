-- Phase 8 correction: comment annotation source attribution
-- Run after:
...
-- Separate nullable FKs for bank items vs assignment-scoped comments.
...
-- At most one source reference may be populated.
do $$
begin
  if not exists (
    select 1
...
-- Backfill text snapshots from existing content / geometry.
...
where annotation_type in ('area_comment', 'text_comment')
  and (
    text_snapshot is null
    or btrim(text_snapshot) = ''
  );

-- Clear orphaned bank-item FKs that were incorrectly written with assignment IDs.
...
-- Persist both source columns via the upsert RPC.
